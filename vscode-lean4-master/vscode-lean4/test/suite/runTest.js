...
function clearUserWorkspaceData(vscodeTest) {
    const workspaceData = path.join(vscodeTest, 'user-data', 'Workspaces');
    fs.rm(workspaceData, { recursive: true }, err => {
        logger_1.logger.log(`deleted user workspace data ${workspaceData} is deleted!`);
    });
}
...
async function main() {
    try {
        ...
        if (process.platform === 'win32') {
            vscodeExecutablePath = await (0, test_electron_1.downloadAndUnzipVSCode)({
        ...
        const [cliPath, ...cliCommonArguments] = (0, test_electron_1.resolveCliArgsFromVSCodeExecutablePath)(vscodeExecutablePath);
        const result = p.spawnSync(cliPath, [...cliCommonArguments, '--install-extension', 'tamasfe.even-better-toml', '--force'], {
            encoding: 'utf-8',
            stdio: 'inherit',
