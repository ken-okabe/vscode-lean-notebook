...
LeanNotebook can display SVG images generated by your Lean code. Your `.lean` file produces SVG files, and `@svg` markers tell the notebook where to display them.
...
-- 1. Define functions that return SVG strings
def circle : String :=
  "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 200 100\">" ++
  "<circle cx=\"100\" cy=\"50\" r=\"40\" fill=\"#3b82f6\"/></svg>"
...
Add this to your `lakefile.lean`, pointing `root` to your `.lean` file:

```lean
lean_exe «svg_gen» where
  root := `MyDocument`    -- matches your file name (MyDocument.lean)
```

Then build and run:

```bash
lake build svg_gen && lake exe svg_gen
```

The extension also runs this automatically when you open the preview.
...
| | |
|:---:|:---:|
| @svg chart_a.svg | @svg chart_b.svg |
| @svg chart_c.svg | @svg chart_d.svg |
-/
```

`@svg` can be mixed freely with Markdown, Mermaid diagrams, and MathJax.

### HTML Export

When exporting to HTML, all SVG content is embedded directly into the output file. The exported HTML is fully self-contained — no external files needed.
